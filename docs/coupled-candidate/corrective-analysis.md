...
| The rejected rows are *"null-space / gradient artefacts that the divergence-free projection did not remove."* | The rows fail the reported energy balance. The cause is **unproven**: `E_ind` is a rank-one surrogate that lower-bounds the true port energy, so either the row is not an eigenvector at its frequency or its port field is strongly non-uniform. Either way its reported `p` is untrustworthy. |
