...
| [`synthetic-three-mode-identifiability.md`](synthetic-three-mode-identifiability.md) | **SYNTHETIC**, offline, no solve: with a third in-band environment mode, fluxonium-site data fix the circuit only up to a rotation of the readout/third-mode block. They identify the environment normal-mode quantities (`Z_FF` pole and residue) exactly; those equal the registered node-basis triple only when `S_RX = 0`, which the Maxwell constraints exclude, and differ by 17 % in `g` on the synthetic circuit. One non-loading readout-side voltage ratio pins the node-basis target uniquely. The next step is a definition decision, not a solve. Record: `experiments/SYNTHETIC-three-mode-identifiability/` |
