...
The runtime image pins the process to one MPI rank, one OpenMP thread and
one OpenBLAS thread, so a run is single-threaded end to end; the timings in
the records are for that configuration.
...
`results/PALACE-GOLDEN-20260915T025408Z/` (run 34922854864 on f6754b0,
commit 2583a45) repeated it again: same image ID, inputs, all four
frequencies to every printed digit, `domain-E.csv` and gate verdicts, but
its `eig.csv` is not byte-identical to the first two. The GMRES residual
