...
class PalaceSolver(SolverAdapter):
    ...
    def preflight(self) -> None:
        ...
        if info.returncode != 0:
            raise SolverUnavailable(
                ...
                f"({(info.stderr or info.stdout).strip().splitlines()[-1:] or ['no output']})",
    ...
    def _image_self_description(self) -> tuple[str | None, str | None]:
        ...
        result = self._runtime(
            [
                ...
                self.image,
                "-c",
                f"cat {IMAGE_VERSION_FILE} 2>/dev/null; echo; cat {IMAGE_COMMIT_FILE} 2>/dev/null",
            ],
            timeout=120,
        )
        ...
        commit = lines[1] if len(lines) > 1 and lines[1] else None
        return version, commit

    def _runtime(self, args: list[str], timeout: int) -> subprocess.CompletedProcess[str]:
        try:
            return subprocess.run(
                [self.runtime, *args], capture_output=True, text=True, timeout=timeout
            )
        except (subprocess.SubprocessError, OSError) as exc:
            return subprocess.CompletedProcess([self.runtime, *args], 1, "", str(exc))
    ...
    def run(self, prepared: PreparedInput) -> PalaceRawOutput:
        ...
        try:
            ...
            log_path.write_text(log_text if isinstance(log_text, str) else log_text.decode())
            raise PalaceRunFailed(
                f"Palace exceeded the {self.timeout_s}s timeout; command: "
    ...
    def _write_run_record(self, raw: PalaceRawOutput) -> None:
        ...
        inputs = {
            Path(p).name: _sha256(p) for p in raw.prepared.input_files if Path(p).exists()
        }
        outputs: dict[str, str] = {}
        if raw.output_dir.exists():
            for path in sorted(raw.output_dir.rglob("*")):
                if path.is_file():
                    outputs[path.relative_to(raw.prepared.work_dir).as_posix()] = _sha256(path)
        if raw.log_path.exists():
            outputs[raw.log_path.name] = _sha256(raw.log_path)
        record = {
            "schema": "qmhp-cem.palace-run/0.2.0",
    ...
    def parse(self, raw_output: PalaceRawOutput) -> SolverResults:
        ...
        convergence = Convergence(
            ...
            metric="eigenmode_backward_error_max",
        ...
        notes = [
            ...
            "Convergence reported is the eigensolver's backward error on this "
            "mesh; a mesh-refinement convergence campaign is deferred (spec §15).",
        ]
    ...
    def _artifacts(self, raw: PalaceRawOutput) -> list[Artifact]:
        work = Path(raw.prepared.work_dir)
        roles = {
