"""openEMS solver adapter (spec §10.4).
...
Spec §10.5 is absolute: if openEMS is requested and cannot execute, the run
...
#: Image built from docker/openems.Dockerfile.
DEFAULT_IMAGE = "qmhp-cem/openems:0.1.0"


@register
class OpenemsSolver(SolverAdapter):
    """openEMS FDTD EM solver."""
    ...
    def preflight(self) -> None:
        ...
        raise SolverUnavailable(
            self.name,
            "the openEMS container invocation is not implemented in QMHP-CEM v0.1",
            f"Build the image from docker/openems.Dockerfile and implement "
            f"OpenemsSolver.run(). Real openEMS validation of Object 001 is "
    ...
    def parse(self, raw_output: Any) -> SolverResults:
        raise SolverUnavailable(
            self.name,
            "openEMS output parsing is not implemented in QMHP-CEM v0.1",
        )
